...
For Niko Home Control I, the `actionId`, `thermostatId` or `energyMeterId` parameter are the unique IP Interface Object ID (`ipInterfaceObjectId`) as automatically assigned in the Niko Home Control Controller when programming the Niko Home Control system using the Niko Home Control I programming software.
...
The same applies applies for the `thermostatId`, `energyMeterId` and `accessId`.
...
```
Thing nikohomecontrol:energymeter:mybridge:mymeter [ energyMeterId="3" ]
```
...
| demand          | R  |    X     | Number             | thermostat  | indicating if the system is actively heating/cooling, same as `heatingdemand` but numeric values (-1=Cooling, 0=None, 1=Heating). This channel is kept for binding backward compatibility |
...
The bridge types have two trigger channels `alarm` and `notice`.
They can be used as a triggers to rules.
...
Bridge nikohomecontrol:bridge:nhc1 [ addr="192.168.0.70", port=8000, refresh=300 ] {
    ...
    energyMeterLive 6 [ energyMeterId="1" ]
    energyWater [ energyMeterId="3" ]
}
